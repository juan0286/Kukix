 ...
 *
 * @param Skill Lost Text Singular
 * @desc Text used to display losted skills singular.
 * @default Acquired Skill
 *
 * @param Skill Lost Text Plural
 * @desc Text used to display losted skills plural.
 * @default Acquired Skills
 *
...
Yanfly.Param.ALUDownTitleFmt = String(Yanfly.Parameters['Level Down Title']);
...
Yanfly.Param.ALULostSkillSing = String(Yanfly.Parameters['Skill Lost Text Singular']);
Yanfly.Param.ALULostSkillPlur = String(Yanfly.Parameters['Skill Lost Text Plural']);
...
Window_VictoryLevelUp.prototype.refresh = function() {
    ...
    if(this._actor._expGained > 0)
      this.drawLearnedSkillsTitle();
    else
      this.drawLostedSkillsTitle();
};
...
Window_VictoryLevelUp.prototype.drawLostedSkillsTitle = function() {
    var total = this._actor._victoryLostSkills.length;
    if (total <= 0) return;
    this.resetFontSettings();
    this.resetTextColor();
    this.changeTextColor(this.systemColor());
    if (total > 1) {
      var text = Yanfly.Param.ALULostSkillPlur;
    } else {
      var text = Yanfly.Param.ALULostSkillSing;
    }
    var rect = this.itemRect(0);
    var wx = rect.x + rect.width;
    var wy = 0;
...
Window_VictorySkills.prototype.makeItemList = function() {
    if(this._actor._expGained > 0)
        ...
        this._data = this._actor._victoryLostSkills.slice();
...
Scene_Battle.prototype.setupNextAftermathLevelUpActor = function() {
    ...
    var fmt = (this._levelUpActor._expGained > 0) ? Yanfly.Param.ALUTitleFmt : Yanfly.Param.ALUDownTitleFmt;    
